...
export default {
  ...
  data() {
    ...
    return {
      ...
      taskFinish: false,
      tasks: list,
      item: { taskID: this.id, doTask: this.newTaskName, taskFinish: false },
  ...
  methods: {
      ...
      axios
        ...
        .then(response => {
          ...
          if (response.data.length !== 0) {
            ...
            this.id = this.tasks[this.tasks.length - 1].taskID+1;
    ...
    goUnDone(url,id){
      ...
      axios.put(`${url}/${id}`,{ itaskID: id, doTask: "a", taskFinish: true })
    },
    goDone(url,id){
      this.tasks[id].taskFinish = true
      axios.put(`${url}/${id}`,{ taskID: id, doTask: "a", taskFinish: true })
    }
  },
  mounted() {
