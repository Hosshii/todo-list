<template>
  <div>
    ...
    <div class="task-list">
      <div class="done">
        <h2>完了</h2>
        <div v-for="(task,a) in JSON.parse(tasks)" :key="`${task.id}`" v-if="task.taskFinish">
  ...
  methods: {
    addTask(url) {
      if (this.newTaskName !== "") {
        ...
        this.tasks.push(jsontask);  //objectの格納
      ...
      axios
        ...
        .then(response => {
          ...
          if (response.data.length !== 0) {
            this.tasks = JSON.parse(response.data);   //返ってくるのはオブジェクトっぽい
