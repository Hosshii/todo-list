    ...
    <div class="task-list">
      <div class="done">
        ...
        <div v-for="(task,a) in tasks" :key="`${task.id}`" v-if="task.taskFinish">
          ...
          <button @click="goUnDone(this.baseUrl,task.id)">未完へ</button>
      ...
      <div class="not-done">
        ...
        <div v-for="(task,a) in tasks" :key="`${task.id}`" v-if="!task.taskFinish">
          <div class="name">タスク:{{task.doTask}}</div>
          <button @click="goDone(this.baseUrl,task.id)">完了！</button>
          <button @click="deleteTask(a)">削除</button>
        </div>
      </div>
    </div>
  ...
  methods: {
    addTask(url) {
      if (this.newTaskName !== "") {
        ...
        axios
          .post(url, this.item)
          .then(console.log(this.id), console.log(this.tasks[this.id]));
    ...
    goUnDone(url,id){
      this.tasks[id-1].taskFinish = false
      axios.put(`${url}\/\:${id}`,false)
    },
    goDone(url,id){
      this.tasks[id-1].taskFinish = true
      axios.put(`${url}\/\:${id}`,true)
    }
  },
  mounted() {
